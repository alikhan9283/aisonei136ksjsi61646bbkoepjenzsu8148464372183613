...
// Public Invidious instances rotate between healthy/CAPTCHA'd/dead
// unpredictably (confirmed via docs.invidious.io — this is a known,
// ongoing issue, not something specific to this bot). A hardcoded list
// goes stale, so instances are instead pulled live from Invidious's own
// official status API before every attempt, and only instances currently
// marked healthy (type "https", API enabled) are tried. Falls back to the
// static list above only if the status API itself is unreachable.
let cachedInstances = null;
let cachedAt = 0;
const CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes

async function getLiveInstances() {
    const now = Date.now();
    if (cachedInstances && (now - cachedAt) < CACHE_TTL_MS) return cachedInstances;

    try {
        const res = await axios.get('https://api.invidious.io/instances.json?sort_by=health', { timeout: 10000 });
        const entries = Array.isArray(res.data) ? res.data : [];
        const healthy = entries
            .map(e => Array.isArray(e) ? e[1] : e) // format is [domain, {details}] pairs
            .filter(d => {
                if (!d) return false;
                const isHttps = d.type === 'https' || d.type === undefined; // some entries omit type
                const apiEnabled = d.api !== false; // default to true unless explicitly disabled
                return isHttps && apiEnabled && d.uri;
            })
            .map(d => d.uri.replace(/\/$/, ''));

        if (healthy.length) {
            cachedInstances = healthy.slice(0, 8); // cap how many we try per request
            cachedAt = now;
            return cachedInstances;
        }
    } catch (e) {
        console.log('[YTHELPER instances.json] failed, using static fallback list:', e.message);
    }
...
async function getStreams(videoId) {
    ...
    // Combine live + static list (deduped) so a stale live-list still falls
    // through to the hardcoded backups instead of giving up early.
    const instances = [...new Set([...liveInstances, ...INVIDIOUS_INSTANCES])];
