...
// Defensive require: if lib/owner.js is missing or its export shape ever
// changes, a bare require() here would throw at file-load time and the
// entire av2.js file would fail to register with the bot — producing
// exactly the "command gives zero response" symptom. Falling back to a
// no-op keeps the file loading; isOwner then relies on the fromMe/self-jid
// checks already present below.
let isOwnerNumber = () => false;
try {
    isOwnerNumber = require('../lib/owner').isOwner || isOwnerNumber;
} catch (e) {
    console.error('AV2: could not load ../lib/owner, falling back to fromMe-only owner check:', e.message);
}
...
cmd({
  ...
  // Fallback in case `reply` isn't provided by the dispatcher for some
  // reason — without this, a missing `reply` would silently swallow every
  // response in this command (which matches the "no response at all"
  // symptom being fixed here).
  if (typeof reply !== 'function') {
    reply = (text) => conn.sendMessage(from, { text }, { quoted: m });
  }
  ...
  try {
    ...
    try {
      ...
      console.error('AV2 attachAutoTrigger error:', attachErr.message);
    }

    // Owner check done entirely inside this file, independent of what the
    // dispatcher passes through. Wrapped defensively since conn.user's
    // shape can vary between bot forks/versions — if it's missing or
    // differently shaped, fall back to fromMe/isOwnerNumber checks instead
    // of throwing and killing the whole command silently.
    let botSelfNum = '';
    try {
        botSelfNum = (conn.user?.id || conn.user?.jid || '').split(':')[0].split('@')[0];
    } catch (e) {
        botSelfNum = '';
    }
    const senderRaw = sender || m.key.participant || m.key.remoteJid || '';
    const senderNum = String(senderRaw).split('@')[0].split(':')[0];
    const isOwner = !!m.key.fromMe || (botSelfNum && senderNum === botSelfNum) || isOwnerNumber(senderNum) || isOwnerNumber(senderRaw);
