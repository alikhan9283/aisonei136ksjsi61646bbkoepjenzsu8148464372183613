...
async function uploadImage(buffer) {
    ...
    // Primary: tmpfiles.org — confirmed via official docs
    // Response: { status: "success", data: { url: "https://tmpfiles.org/{id}/{name}" } }
    // Note: that URL is a preview page — the direct file needs "/dl/" inserted.
    try {
        ...
        if (previewUrl) {
            const directUrl = previewUrl.replace("tmpfiles.org/", "tmpfiles.org/dl/");
            return directUrl;
        }
        errors.push("tmpfiles.org: no url in response");
    } catch (e) {
        errors.push(`tmpfiles.org: ${e.response?.status || ''} ${e.message}`);
    }
    ...
    try {
        ...
        errors.push("catbox: did not return a usable URL");
    } catch (e) {
        errors.push(`catbox: ${e.response?.status || ''} ${e.message}`);
    }
