...
// people). More energetic/action-y moves.
const categories = ['punch', 'kick', 'run', 'thumbsup', 'highfive'];
...
async function convertGifToMp4(gifBuffer) {
    const inputPath = path.join('/tmp', `bv_in_${Date.now()}.gif`);
    const outputPath = path.join('/tmp', `bv_out_${Date.now()}.mp4`);
