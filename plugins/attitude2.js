...
async function fetchAttitude() {
    ...
    // Source 1: waifu.pics smug (confirmed valid tag)
    try {
    ...
    // Source 2: waifu.pics bully (confident/teasing poses, same family)
    try {
        const res = await axios.get('https://api.waifu.pics/sfw/bully', AXIOS_DEFAULTS);
        if (res.data?.url) return { url: res.data.url, source: 'waifu.pics-bully' };
    } catch (e) { errors.push(`waifu.pics-bully: ${e.message}`); }
    ...
    console.error('[ATTITUDE] All sources failed:', errors.join(' | '));
    throw new Error('All sources are down right now — try again shortly.');
}

cmd({
    pattern: 'attitude2',
    alias: ['attitudeanime2', 'attipic2'],
    ...
    try {
        ...
        const { url: imgUrl, source } = await fetchAttitude();
...
‎*╰───────◉◉◉────━┈៚*`;
        ...
        await conn.sendMessage(from, { react: { text: '❌', key: mek.key } });
        reply(`⚠️ Error fetching attitude image: ${e.message}`);
    }
});
